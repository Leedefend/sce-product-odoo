# -*- coding: utf-8 -*-
"""Probe load_contract Lite preview through the Odoo handler boundary.
...
from odoo import SUPERUSER_ID, api  # noqa: F401
...
def _run_handler(params):
    handler = LoadContractHandler(env, payload={"params": params})  # noqa: F821
    ...
    params.update(
        {
            ...
            "traceId": "lite-load-contract-interface-001",
        }
    )
    return params
...
_assert(default_response.get("status") == "success", errors, "default response must be success")
_assert("lite_preview" not in default_response, errors, "default response must not include lite_preview")
_assert(incomplete_response.get("status") == "success", errors, "incomplete opt-in response must be success")
_assert("lite_preview" not in incomplete_response, errors, "incomplete opt-in must not include lite_preview")
_assert(valid_response.get("status") == "success", errors, "valid opt-in response must be success")
...
if isinstance(preview, dict):
    ...
    _assert(meta.get("traceId") == "lite-load-contract-interface-001", errors, "preview traceId must be preserved")
...
report = {
    ...
    "model": "project.project",
    "default_status": default_response.get("status"),
...
out = _artifact_root() / "unified_page_contract_lite_load_contract_preview_interface.json"
out.write_text(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
print(json.dumps(report, ensure_ascii=False, indent=2, sort_keys=True))
print("UNIFIED_PAGE_CONTRACT_LITE_LOAD_CONTRACT_INTERFACE_REPORT=%s" % out)
if errors:
    raise SystemExit(1)
print("UNIFIED_PAGE_CONTRACT_LITE_LOAD_CONTRACT_INTERFACE=PASS")
