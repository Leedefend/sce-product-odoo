...
ensure_module_installed() {
  ...
  echo "[verify.e2e] DB ${DB_NAME} missing module ${module} (state=${state})"
  if [ "${E2E_AUTO_INSTALL:-}" = "1" ]; then
    echo "[verify.e2e] auto-install enabled, installing ${module}"
    CODEX_NEED_UPGRADE=1 MODULE="${module}" ODOO_ARGS="-i ${module}" make mod.upgrade
    make restart
    return 0
  fi

  echo "[verify.e2e] run: CODEX_NEED_UPGRADE=1 MODULE=${module} ODOO_ARGS=\"-i ${module}\" make mod.upgrade"
  return 2
}

ensure_module_installed "smart_core"

# frontend_api lives in smart_construction_core
ensure_module_installed "smart_construction_core"

echo "[verify.e2e] prerequisites OK"
