...
ensure_module_installed() {
  ...
  echo "[verify.e2e.scene] DB ${DB_NAME} missing module ${module} (state=${state})"
  if [ "${E2E_AUTO_INSTALL:-}" = "1" ]; then
    echo "[verify.e2e.scene] auto-install enabled, installing ${module}"
    CODEX_NEED_UPGRADE=1 MODULE="${module}" ODOO_ARGS="-i ${module}" make mod.upgrade
    make restart
    return 0
  fi

  echo "[verify.e2e.scene] run: CODEX_NEED_UPGRADE=1 MODULE=${module} ODOO_ARGS=\"-i ${module}\" make mod.upgrade"
  return 2
}
...
echo "[verify.e2e.scene] prerequisites OK"
