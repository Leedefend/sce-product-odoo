#!/usr/bin/env python3
"""Guard the wx_mini Lite renderer-input pilot path."""
...
def main() -> int:
    ...
    boundary = create_boundary(wx_contract) if wx_contract else {}
    renderer_input = create_renderer_input(boundary) if boundary else {}

    if renderer_input.get("clientType") != "wx_mini":
        errors.append("renderer input clientType must be wx_mini")
    ...
    if renderer_input.get("viewType") not in {"tree", "list"}:
        errors.append(f"wx_mini pilot must use tree/list view, got {renderer_input.get('viewType')!r}")
    if renderer_input.get("widgetCount", 0) < 1:
        errors.append("wx_mini renderer input must expose at least one widget")
    if renderer_input.get("fieldCount", 0) < 1:
        errors.append("wx_mini renderer input must expose at least one field")
    if renderer_input.get("actionCount", 0) < 1:
        errors.append("wx_mini renderer input must expose at least one action")
    ...
    report = {
        ...
        "decision": "wx_mini_renderer_input_ready_ui_renderer_pending" if not errors else "blocked",
        "errors": errors,
    }
    write_report(args.report, report)

    if errors:
        print("Unified Semantic Page Contract Lite wx_mini renderer input pilot failed:")
        for error in errors:
            print(f"- {error}")
        print(f"- report: {args.report}")
        return 1

    print("Unified Semantic Page Contract Lite wx_mini renderer input pilot passed")
    print("- decision: wx_mini_renderer_input_ready_ui_renderer_pending")
    print(f"- report: {args.report}")
    return 0
