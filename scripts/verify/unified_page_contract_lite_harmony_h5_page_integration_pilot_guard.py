#!/usr/bin/env python3
"""Guard the harmony_h5 Lite page integration pilot path."""
...
def main() -> int:
    ...
    renderer_output = create_renderer_output(harmony_contract) if harmony_contract else {}
    page_integration = create_page_integration(renderer_output) if renderer_output else {}

    if page_integration.get("clientType") != "harmony_h5":
        errors.append("page integration clientType must be harmony_h5")
    ...
    if page_integration.get("viewType") not in {"tree", "list"}:
        errors.append(f"harmony_h5 page integration pilot must use tree/list view, got {page_integration.get('viewType')!r}")
    ...
    report = {
        ...
        "decision": "harmony_h5_page_integration_pilot_ready_runtime_mount_pending" if not errors else "blocked",
        "errors": errors,
    }
    write_report(args.report, report)

    if errors:
        print("Unified Semantic Page Contract Lite harmony_h5 page integration pilot failed:")
        for error in errors:
            print(f"- {error}")
        print(f"- report: {args.report}")
        return 1

    print("Unified Semantic Page Contract Lite harmony_h5 page integration pilot passed")
    print("- decision: harmony_h5_page_integration_pilot_ready_runtime_mount_pending")
    print(f"- report: {args.report}")
    return 0
