#!/usr/bin/env python3
"""Guard the wx_mini Lite UI renderer pilot path."""
...
def main() -> int:
    ...
    renderer_input = create_renderer_input(wx_contract) if wx_contract else {}
    renderer_output = create_renderer_output(renderer_input) if renderer_input else {}

    if renderer_output.get("clientType") != "wx_mini":
        errors.append("renderer output clientType must be wx_mini")
    ...
    if renderer_output.get("viewType") not in {"tree", "list"}:
        errors.append(f"wx_mini UI renderer pilot must use tree/list view, got {renderer_output.get('viewType')!r}")
    ...
    report = {
        ...
        "decision": "wx_mini_ui_renderer_pilot_ready_page_integration_pending" if not errors else "blocked",
        "errors": errors,
    }
    write_report(args.report, report)

    if errors:
        print("Unified Semantic Page Contract Lite wx_mini UI renderer pilot failed:")
        for error in errors:
            print(f"- {error}")
        print(f"- report: {args.report}")
        return 1

    print("Unified Semantic Page Contract Lite wx_mini UI renderer pilot passed")
    print("- decision: wx_mini_ui_renderer_pilot_ready_page_integration_pending")
    print(f"- report: {args.report}")
    return 0
