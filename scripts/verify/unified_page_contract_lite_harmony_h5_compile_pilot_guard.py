#!/usr/bin/env python3
"""Guard the harmony_h5 Lite real compile pilot."""
...
def main() -> int:
    ...
    if "verify.unified_page_contract.lite.harmony_h5_compile_pilot.host" not in makefile:
        errors.append("Makefile target missing: verify.unified_page_contract.lite.harmony_h5_compile_pilot.host")
    ...
    observations.append(
        "frontend lockfile includes mobile workspace"
        if lock_has_mobile
        else "frontend lockfile missing mobile workspace entries"
    )
    observations.append(
        "frontend lockfile includes UniApp dependencies"
        if lock_has_uniapp
        else "frontend lockfile missing UniApp dependencies"
    )
    observations.append(
        "frontend lockfile includes UniApp H5 dependency"
        if lock_has_h5
        else "frontend lockfile missing UniApp H5 dependency"
    )
    observations.append("mobile node_modules exists" if deps_ready else "mobile node_modules missing")
    observations.append(f"pnpm available: {pnpm_bin}" if pnpm_bin else "pnpm missing")
    ...
    if args.execute and can_execute:
        ...
        if completed.returncode != 0:
            errors.append("harmony_h5 compile command failed")

    if errors:
        decision = "harmony_h5_compile_pilot_failed"
    elif args.execute and compile_result and compile_result.get("returnCode") == 0:
        decision = "harmony_h5_compile_pilot_passed"
    elif not can_execute:
        decision = "harmony_h5_compile_pilot_blocked_missing_lockfile_or_dependencies"
    else:
        decision = "harmony_h5_compile_pilot_ready_not_executed"
    ...
    if errors:
        print("Unified Semantic Page Contract Lite harmony_h5 compile pilot failed:")
        for error in errors:
            print(f"- {error}")
        print(f"- report: {args.report}")
        return 1

    print("Unified Semantic Page Contract Lite harmony_h5 compile pilot checked")
