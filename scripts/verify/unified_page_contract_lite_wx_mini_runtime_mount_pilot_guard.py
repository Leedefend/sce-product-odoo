#!/usr/bin/env python3
"""Guard the wx_mini Lite runtime mount pilot path."""
...
def main() -> int:
    ...
    page_integration = create_page_integration(wx_contract) if wx_contract else {}
    runtime_mount = create_runtime_mount(page_integration) if page_integration else {}

    if runtime_mount.get("clientType") != "wx_mini":
        errors.append("runtime mount clientType must be wx_mini")
    ...
    if runtime_mount.get("viewType") not in {"tree", "list"}:
        errors.append(f"wx_mini runtime mount pilot must use tree/list view, got {runtime_mount.get('viewType')!r}")
    ...
    report = {
        ...
        "decision": "wx_mini_runtime_mount_pilot_ready_compile_pending" if not errors else "blocked",
        "errors": errors,
    }
    write_report(args.report, report)

    if errors:
        print("Unified Semantic Page Contract Lite wx_mini runtime mount pilot failed:")
        for error in errors:
            print(f"- {error}")
        print(f"- report: {args.report}")
        return 1

    print("Unified Semantic Page Contract Lite wx_mini runtime mount pilot passed")
    print("- decision: wx_mini_runtime_mount_pilot_ready_compile_pending")
    print(f"- report: {args.report}")
    return 0
