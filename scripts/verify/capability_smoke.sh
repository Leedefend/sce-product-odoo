...
ensure_module_installed() {
  ...
  echo "[capability_smoke] DB ${DB_NAME} missing module ${module} (state=${state})"
  if [ "${E2E_AUTO_INSTALL:-}" = "1" ]; then
    echo "[capability_smoke] auto-install enabled, installing ${module}"
    CODEX_NEED_UPGRADE=1 MODULE="${module}" ODOO_ARGS="-i ${module}" make mod.upgrade
    make restart
    return 0
  fi

  echo "[capability_smoke] run: CODEX_NEED_UPGRADE=1 MODULE=${module} ODOO_ARGS=\"-i ${module}\" make mod.upgrade"
  return 2
}
...
python3 "$ROOT_DIR/scripts/verify/capability_smoke.py"
