...
compose_dev exec -T \
  ...
  -e APPLY="${APPLY:-0}" \
  -e FINANCE_LEGACY_TREASURY_CURRENCY_ALLOW_DB="${FINANCE_LEGACY_TREASURY_CURRENCY_ALLOW_DB:-0}" \
  odoo odoo shell -d "$DB_NAME" \
  --db_host=db --db_port=5432 --db_user="$DB_USER" --db_password="$DB_PASSWORD" \
  --addons-path="$ODOO_ADDONS_PATH" \
  --logfile=/dev/null --log-level=warn \
  < "$ROOT_DIR/scripts/ops/finance_legacy_treasury_currency_backfill.py"
