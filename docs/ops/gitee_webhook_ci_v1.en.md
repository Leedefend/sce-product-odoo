...
2. Verify the signature and timestamp with Gitee's documented algorithm, accepting both the documented headers and the API-created hook's `sign`/`timestamp` query transport. Validate freshness and HMAC independently for both complete transports and accept only when at least one passes. A dedicated parser preserves Gitee's unescaped base64 `+`; the clock skew is 300 seconds, and query signatures are excluded from access logs.
...
The 17-case matrix covers positive header/query signature transport, raw base64 `+`, valid dual-transport fallback, Push-to-PR queue upgrades, receiver/worker secret isolation, strictly scoped interrupted-workspace cleanup, invalid signatures, unexpected or repeated query parameters, expiry, replay, wrong repository, wrong sender, fork PRs, branch/command injection, deleted/closed events, and secret isolation.
