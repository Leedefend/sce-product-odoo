...
2. 按 Gitee 官方算法校验签名与时间戳，兼容文档请求头和 API 创建钩子的
   `sign`/`timestamp` 查询参数；双通道分别做完整新鲜度与 HMAC 校验，至少一个通过才接收。
...
17 项矩阵包括请求头/查询签名正向用例、原始 Base64 `+`、双通道有效回退、Push→PR 队列升级、receiver/worker 密钥隔离、严格限定的中断工作区清理，以及无效签名、意外或重复查询参数、过期请求、重放、错误仓库、错误发送者、fork PR、分支/命令注入、删除/关闭事件和 secret 环境隔离。
