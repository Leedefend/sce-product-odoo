...
- System-bound verification: nine signed WebHook positive/negative tests, server systemd activation, loopback and trusted public HTTPS health probes, unsigned public request denial, fixed repository/sender allowlist, replay denial, fork denial, SHA injection denial, secret environment isolation, and pre-write Gitee scope validation
- External state: Nginx 1.24 and Certbot 5.7 are active; Let's Encrypt issued the short-lived IP certificate and the renewal timer is enabled. Repository writes remain pending because the supplied valid token lacks the current granular `keys`, `hook`, and `pull_requests` scopes.
